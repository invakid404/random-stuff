...
type DistributeUnions<T> = T extends any ? DistributeOne<T> : never;

type DistributeOne<T> = T extends readonly any[]
  ? DistributeTuple<T>
  : T extends Record<string, unknown>
  ? DistributeRecord<T>
  : T;

type DistributeTuple<T extends readonly any[]> = T extends [
  infer Head,
  ...infer Rest,
]
  ? DistributeUnions<Head> extends infer DistributedHead
    ? DistributedHead extends any
      ? [DistributedHead, ...DistributeTuple<Rest>]
      : never
    : never
  : [];
...
type FromEntries<Entries extends [PropertyKey, unknown][]> = {
  [Key in Entries[number] as Key[0]]: Key[1];
};

type DistributeRecord<
  T extends Record<string, unknown>,
  Keys = keyof T,
  Entries extends [string, unknown][] = [],
> = [Keys] extends [never]
  ? FromEntries<Entries>
  : LastInUnion<Keys> extends infer Key extends string
  ? DistributeUnions<T[Key]> extends infer DistributedValue
    ? DistributedValue extends any
      ? DistributeRecord<
          T,
          Exclude<Keys, Key>,
          [...Entries, [Key, DistributedValue]]
        >
      : never
    : never
  : never;
