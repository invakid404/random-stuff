...
type Characters<Value extends string> = Split<Value, ''>;

type LowercaseLetters = 'abcdefghijklmnopqrstuvwxyz';
...
type CapitalizeWordsRec<S extends string> =
  S extends `${infer First}${infer Rest}`
    ? CapitalizeWordsRec<Rest> extends infer Result extends string
      ? First extends Letter
        ? `${First}${Result}`
        : `${First}${Capitalize<Result>}`
      : never
    : S;

type CapitalizeWords<S extends string> = Capitalize<CapitalizeWordsRec<S>>;
