...
type CountElementNumberToObject<T extends readonly unknown[]> =
  CountElementsHelper<T> extends infer Result extends Record<
    PropertyKey,
    readonly unknown[]
  >
...
type CountElementsHelper<
  T extends readonly unknown[],
  Acc extends Record<PropertyKey, readonly unknown[]> = {},
> = T extends [infer Head, ...infer Rest]
  ? [Head] extends [never]
    ? CountElementsHelper<Rest, Acc>
    : Head extends unknown[]
    ? CountElementsHelper<Rest, CountElementsHelper<Head, Acc>>
    : Head extends PropertyKey
    ? CountElementsHelper<
        Rest,
        Omit<Acc, Head> & {
          [Key in Head]: Head extends keyof Acc
            ? [...Acc[Head], unknown]
            : [unknown];
        }
      >
