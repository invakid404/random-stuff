...
type Permutation<T extends string, K extends string = T> = [T] extends [never]
  ? ''
  : K extends K
  ? Permutation<Exclude<T, K>> extends infer Current extends string
    ? Current | `${K}${Current}`
    : never
  : never;
...
type AllCombinations<S extends string> = Permutation<StringToUnion<S>>;
