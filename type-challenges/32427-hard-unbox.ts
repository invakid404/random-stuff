...
type Depth = keyof Predecessor;

type Unbox<T, D extends Depth = 9> = UnboxHelper<
  T,
  D extends 0 ? 9 : Predecessor[D]
>;

type UnboxHelper<T, D extends Depth> = [D] extends [never]
  ? T
  : UnboxOne<T> extends infer U
    ? [T] extends [U]
      ? U
      : UnboxHelper<U, Predecessor[D]>
    : never;

type UnboxOne<T> = T extends PromiseLike<infer U>
  ? U
...
type cases = [
  ...
  Expect<
    Equal<Unbox<Promise<Promise<Promise<number>>>, 1>, Promise<Promise<number>>>
  >,
