...
type IsFixedStringLiteralType<S extends string> = IsUnionOrNever<S> extends true
  ? false
  : MyEqual<S, string> extends true
    ? false
    : S extends `${infer First}${infer Rest}`
      ? true extends
          | MyEqual<First, string>
          | MyEqual<First, `${number}`>
          | MyEqual<First, `${bigint}`>
          | MyEqual<First, `${any}`>
        ? false
...
type IsUnionOrNever<T, U = T> = [T] extends [never]
  ? true
  : T extends any
    ? [U] extends [T]
      ? false
