...
type ValidateRows<T extends Digits[][]> = MyEqual<
  {
    [R in keyof T]: MyEqual<T[R][number], Digits>;
  }[number],
  true
>;

type ValidateCols<T extends Digits[][]> = ValidateRows<Transpose<T>>;
...
> = R extends [
  ...
  ? MyEqual<CollectRegion<T, Row, Col>, Digits> extends false
    ? false
...
type SudokuSolved<T extends Digits[][][]> =
  FlattenSudoku<T> extends infer F extends Digits[][]
...
type FlattenSudoku<T extends Digits[][][]> = {
