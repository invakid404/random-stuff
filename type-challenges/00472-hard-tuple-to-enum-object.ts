...
type Enum<T extends readonly string[], N extends boolean = false> = {
  readonly [key in keyof T as T[key] extends string
    ? Capitalize<T[key]>
    : never]: N extends true ? StringToNumber<key & string> : T[key & string];
};

type StringToNumber<
  S extends string,
  Accumulator extends readonly any[] = [],
> = S extends `${Accumulator['length']}`
  ? Accumulator['length']
  : StringToNumber<S, [...Accumulator, any]>;

type Test = Enum<['hello', 'world'], true>;
