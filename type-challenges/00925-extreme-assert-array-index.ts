...
type Hash<Value extends string> = HashRec<Lowercase<Value>>;

type HashRec<
  Value extends string,
  Accumulator extends readonly any[] = [],
> = Value extends `${infer Head}${infer Rest}`
  ? HashRec<Rest, [...Accumulator, ...CharToTuple[Head & keyof CharToTuple]]>
  : Value extends ''
  ? Accumulator['length']
  : never;

type IsArray<Array extends readonly any[]> = number extends Array['length']
  ? true
  : false;

type Hashable<Value extends string> = [Hash<Value>] extends [never]
  ? false
  : true;
...
> = number extends Array['length']
  ? Hash<Key> extends infer HashedKey extends number
    ? Array & { readonly [hashSymbol]: HashedKey } & {
        readonly [K in HashedKey]: Array[number];
      }
    : never
  : never;

function assertArrayIndex<Array extends readonly any[], Key extends string>(
  _array: IsArray<Array> extends true ? Array : never,
  _key: Hashable<Key> extends true ? Key : never,
): asserts _array is IsArray<Array> extends true
  ? TaggedArray<Array, Key>
  : never {}

type Index<Array extends { readonly [hashSymbol]: number }> =
  Array[typeof hashSymbol];
