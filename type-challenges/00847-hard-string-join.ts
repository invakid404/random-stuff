...
declare function join<Delimiter extends string>(
  delimiter: Delimiter,
): <Args extends string[]>(...parts: Args) => Join<Delimiter, Args>;

type Join<Delimiter extends string, Args extends string[]> = Args extends [
  infer Single extends string,
]
  ? Single
  : Args extends [infer Head extends string, ...infer Rest extends string[]]
  ? `${Head}${Delimiter}${Join<Delimiter, Rest>}`
