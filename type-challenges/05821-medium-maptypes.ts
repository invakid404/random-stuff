...
type MapOne<T, R extends TypeMapper> = (
  R extends unknown ? (T extends R['mapFrom'] ? R['mapTo'] : never) : never
) extends infer MappedType
  ? [MappedType] extends [never]
    ? T
    : MappedType
  : never;

type MapTypes<T extends Record<string, unknown>, R extends TypeMapper> = {
  [Key in keyof T]: MapOne<T[Key], R>;
};
