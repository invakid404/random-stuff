...
type IntersectTwo<A, B> = [A, B] extends [object, object]
  ? Omit<A & B, never>
  : A & B;
...
> = Left extends [] ? true : Right extends [] ? true : false;

type IntersectParametersHelper<
  Left extends readonly unknown[],
  Right extends readonly unknown[],
  Acc extends readonly unknown[] = [],
> = EitherEmpty<Left, Right> extends true
  ? [...Acc, ...Left, ...Right]
  : [Left, Right] extends [
        readonly [(infer L)?, ...infer LRest],
        readonly [(infer R)?, ...infer RRest],
      ]
    ? // If either is a tuple
      '0' extends keyof Left | keyof Right
      ? IntersectParametersHelper<
          LRest,
          RRest,
          // If both tuples could be empty, i.e. if the last element of both are optional
          [[], []] extends [Left, Right]
            ? [...Acc, IntersectTwo<L, R>?]
            : [...Acc, IntersectTwo<L, R>]
        >
      : [...Acc, ...IntersectTwo<L, R>[]]
    : never;
...
type IntersectParameters<
  Left extends readonly unknown[],
  Right extends readonly unknown[],
> = IntersectParametersHelper<Left, Right>;
