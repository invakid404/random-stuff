...
> = Defer<
  Acc['length'] extends H
    ? Result<H | Res>
    : Range<L, H, [...Acc, 1], Acc['length'] | Res>
>;

type For<T> = T[Extract<'next', keyof T>];

type GetNext5Times<T> = For<T> extends infer T
...
type GetNext50Times<T> = GetNext5Times<T> extends infer T
  ? GetNext5Times<T> extends infer T
    ? GetNext5Times<T> extends infer T
      ? GetNext5Times<T> extends infer T
        ? GetNext5Times<T> extends infer T
          ? GetNext5Times<T> extends infer T
...
type GetNext200Times<T> = GetNext50Times<T> extends infer T
  ? GetNext50Times<T> extends infer T
    ? GetNext50Times<T> extends infer T
      ? GetNext50Times<T>
      : never
    : never
...
type NumberRange<L extends number, R extends number> = GetNext200Times<
  Range<L, R>
>['result'];
