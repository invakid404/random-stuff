...
type HandlePrimitiveType<T, Type extends keyof PrimitiveTypes> = T extends {
  enum: infer Enum extends readonly unknown[];
}
  ? Enum extends Array<PrimitiveTypes[Type]>
    ? Enum[number]
    : never
  : PrimitiveTypes[Type];
...
type HandleObject<T> = T extends {
  ...
  required?: infer Required extends string[];
}
  ? {
      [Key in keyof Properties]?: JSONSchema2TS<Properties[Key]>;
    } extends infer Object
    ? string[] extends Required
      ? Object
      : Required extends Array<keyof Object>
      ? RequiredKeys<Object, Required[number]>
      : never
    : never
  : Record<string, unknown>;

type HandleArray<T> = T extends { items: infer Items }
  ? Array<JSONSchema2TS<Items>>
  : unknown[];

type Id<T> = { [Key in keyof T]: T[Key] };

type RequiredKeys<T, Keys extends keyof T> = Id<
