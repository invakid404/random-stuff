...
type ReturnTypes<Functions> = {
  [Key in keyof Functions]: Functions[Key] extends (
    ...args: any[]
  ) => infer Return
    ? Return
    : never;
...
declare function defineStore<State, Getters, Actions>(store: {
  ...
  getters: Getters &
    ThisType<
      Readonly<State> & // Getters cannot modify state
        Readonly<ReturnTypes<Getters>> // Getters can call each other
    >;
  actions: Actions &
    ThisType<
      State & // Actions can modify state
        Readonly<ReturnTypes<Getters>> & // Actions cannot assign values to getters
        Readonly<Actions> // Actions can call each other
    >;
}): Readonly<State & ReturnTypes<Getters> & Actions>; // Result is a readonly intersection of all three types
