...
type DefinedPartial<T extends Record<PropertyKey, any>> =
  GenerateAllCombinations<keyof T> extends infer Keys extends PropertyKey[]
    ? {
        [Key in keyof Keys]: Record<Keys[Key], T[keyof T]>;
      }[number]
    : never;

type GenerateAllCombinations<T extends PropertyKey> =
  UnionToTuple<T> extends infer T extends PropertyKey[]
    ? GenerateAllCombinationsHelper<T> extends infer R extends PropertyKey[][]
      ? {
          [K1 in keyof R]: R[K1][number];
        }
      : never
    : never;

type GenerateAllCombinationsHelper<
...
> = T extends [
  ...
  ? [
      ...GenerateAllCombinationsHelper<Rest, Acc>,
      ...GenerateAllCombinationsHelper<Rest, [...Acc, Head]>,
    ]
  : [] extends Acc
    ? []
    : [Acc];

type UnionToTuple<U, Acc extends unknown[] = []> = [U] extends [never]
  ? Acc
  : LastInUnion<U> extends infer Last
    ? UnionToTuple<Exclude<U, Last>, [Last, ...Acc]>
    : never;
