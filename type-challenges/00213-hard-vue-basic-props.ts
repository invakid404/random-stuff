...
type ConstructorToType<T> = T extends undefined
  ? any
  ...
  ? ConstructorToType<T[number]>
  : T extends { prototype: infer Type }
  ? Type
  : any;

type VueProps<Props> = {
  [key in keyof Props]: Props[key] extends { type: infer Constructor }
    ? ConstructorToType<Constructor>
    : ConstructorToType<Props[key]>;
};

declare function VueBasicProps<Props, Data, Computed, Methods>(
  options: {
    props: Props;
    data: (this: VueProps<Props>) => Data;
    computed: Computed & ThisType<Data & Computed>;
    methods: Methods &
      ThisType<Data & Methods & ReturnTypes<Computed> & VueProps<Props>>;
  } & ThisType<{}>
): any;
