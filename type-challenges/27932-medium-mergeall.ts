...
type MergeAll<XS extends readonly unknown[]> =
  MergeAllHelper<XS> extends infer R ? { [Key in keyof R]: R[Key] } : never;

type MergeAllHelper<XS extends readonly unknown[], Acc = {}> = XS extends [
  infer Head,
  ...infer Rest,
]
  ? MergeAllHelper<
      Rest,
      Omit<Acc, keyof Head> & {
        [Key in keyof Head]: Key extends keyof Acc
          ? Head[Key] | Acc[Key]
          : Head[Key];
      }
    >
