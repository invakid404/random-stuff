...
type LengthDifference<
  Left extends readonly unknown[],
  Right extends readonly unknown[],
  Acc extends readonly unknown[] = [],
> = Left['length'] extends Right['length']
  ? Acc['length']
  : Acc['length'] extends Left['length']
  ? never
  : LengthDifference<Left, [...Right, unknown], [...Acc, unknown]>;

type Subtract<M extends number, S extends number> = LengthDifference<
  ConstructTuple<M>,
  ConstructTuple<S>
>;
